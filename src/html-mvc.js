  ...
  return Object.create(null, {
    ...
    'initialize': {
      value: function (object) {
        _record = record(data(object));
      }
    },

    'merge': {
      value: function (object) {
        _record = merge(_record, record(data(object)));
      }
    },

    'record': {
      value: function () {
        return _record['interface'];
      }
    }
      ...
      value: function (record) {
        function bind(element, record) {
          ...
          if (bindText = element.bindText) {
            ...
            do {
              if (child.tagName === 'VIEW') {
                var model, modelName, scope;
                modelName = child.model;
                if (modelName) {
                  model = document.mvc.getModel(modelName);
                  bind(child, model.record());
                  continue;
                }
                scope = child.scope;
                if (scope) {
                  bind(child, record.scope(scope));
                  continue;
                }
                bind(child, record);
              }
...
(function extendFormSubmission() {
  ...
  function parseModelNameMimeTypeExtension(extension) {
    var modelNameRegexp = /^\s*model="?([A-Za-z_-]+)"?/;
    var matches = modelNameRegexp.exec(extension);
    return matches ? matches[1] : undefined;
  }

  // Naivety central, yo
  function parseMultipartResponse(body, boundary) {
    var splitted = body.split(boundary);
    var results = {};
    parse_response_parts:
      for (var i = 0; i < splitted.length; i++) {
        if ((i === splitted.length - 1) && splitted[i] === '--') break;
        var splitMessage = splitted[i].split('\r\n\r\n');
        var header = splitMessage[0], body = splitMessage[1];
        var headers = header.split('\r\n');
        var type;
        parse_content_type:
          for (var j = 0; j < headers.length; j++) {
            var matches = /^Content-Type:\s+(\w+)$/.exec(headers[i]);
            if (!matches) continue parse_content_type;
            var type = matches[1];
            matches = /^application\/json;(\w+)$/.exec(type);
            if (!matches) continue parse_response_parts;
            exts = matches[1].split(';');
            var modelName;
            parse_type_extensions:
              for (var k = 0; k < exts.length; k++) {
                var ext = exts[k];
                var modelNameResult = parseModelNameMimeTypeExtension(ext);
                if (modelNameResult) {
                  modelName = modelNameResult;
                  continue parse_type_extensions;
                }
              }
            if (!modelName) continue parse_response_parts;
          }
      }
    return results;
  }
  ...
  function handleFormResponse(req, fallbackAction) {
    ...
    var matches;
    // Naive regex
    if (!(matches = /^multipart\/mixed;boundary=(.+)$/.exec(type))) {
      window.location.href = fallbackAction;
      return;
    }
    var boundary = '--' + matches[1];
    var result = parseMultipartResponse(req.responseText, boundary);
  }
